 ...
 */
export async function inflate(dirPath: string, repoRootPath: string): Promise<void> {
	...
	try {
			...
			if (fileHash && basename) {
				...
				try {
					...
					try {
						...
						console.log(`Symbolic link for '${basename}' already exists. Skipping.`);
		...
		if (error.code === 'ENOENT') {
			console.log(`No dependencies file found at '${dependenciesPath}'. Skipping inflate.`);
		} else {
			console.error(`Error during inflate operation for '${dirPath}': ${error}`);
			throw error;
		}
	}
}
